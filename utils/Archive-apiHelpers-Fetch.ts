...
export async function POST_createBankAccountWithFetch(ctx: UserData, param_bankName?: string, param_accountNumber?: string, param_routingNumber?: string, param_userID?: string) {
  ...
  try {
    ...
    if (userID && result.userId !== userID) throw new Error(`Expected userId '${userID}' but got '${result.userId}'`)
    if (result.bankName !== bankName) throw new Error(`Expected bankName '${bankName}' but got '${result.bankName}'`)
    if (result.accountNumber !== accountNumber) throw new Error(`Expected accountNumber '${accountNumber}' but got '${result.accountNumber}'`)
    if (result.routingNumber !== routingNumber) throw new Error(`Expected routingNumber '${routingNumber}' but got '${result.routingNumber}'`)
  } catch (error: any) {
    console.error('API request failed:', error.message)
    throw error
  }
